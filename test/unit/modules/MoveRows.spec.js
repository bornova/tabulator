...
describe('MoveRows module', () => {
	...
	it('should handle table row drop event', () => {
		...
		jest.spyOn(moveRowsMod, 'tableRowDrop').mockImplementation((event, toRow) => {
			...
			// Dispatch event
			moveRowsMod.dispatchExternal(
				'movableRowsReceived',
				moveRowsMod.connectedRow.getComponent(),
				toRow ? toRow.getComponent() : undefined,
				moveRowsMod.connectedTable
			)
		...
		expect(moveRowsMod.dispatchExternal).toHaveBeenCalledWith(
			'movableRowsReceived',
			expect.anything(),
			expect.anything(),
			connectedTable
		...
		jest.spyOn(moveRowsMod, 'dropComplete').mockImplementation((table, row, success) => {
			if (success) {
				...
				moveRowsMod.dispatchExternal(
					...
					moveRowsMod.moving.getComponent(),
					row.getComponent(),
					table
				)
			}
