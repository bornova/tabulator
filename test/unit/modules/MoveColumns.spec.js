...
describe('MoveColumns module', () => {
	...
	beforeEach(() => {
		...
		mockTable = {
			modExists: jest.fn((name) => name === 'selectRange'),
			modules: {
				selectRange: {
	...
	it('should start a move operation on mouse down after timeout', () => {
		...
		const mousedownHandler = mockColumn
			.getElement()
			.addEventListener.mock.calls.find((call) => call[0] === 'mousedown')[1]
	...
	it('should finalize column move on mouse up', () => {
		...
		expect(moveColumns.placeholderElement.parentNode.insertBefore).toHaveBeenCalledWith('columnElement', 'nextSibling')
		expect(moveColumns.placeholderElement.parentNode.removeChild).toHaveBeenCalledWith(moveColumns.placeholderElement)
		expect(moveColumns.hoverElement.parentNode.removeChild).toHaveBeenCalledWith(moveColumns.hoverElement)
