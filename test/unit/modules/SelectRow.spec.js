...
describe('SelectRow module', () => {
	...
	beforeEach(() => {
		...
		// Create mock rows
		mockRows = [createMockRow(1), createMockRow(2), createMockRow(3)]
		...
		jest
			.spyOn(SelectRow.prototype, 'registerComponentFunction')
			.mockImplementation(function (component, name, callback) {
	...
	function createMockRow(id) {
		...
		const mockComponent = {
			getData: jest.fn(() => ({ id, name: `Row ${id}` }))
		}
	...
	it('should select multiple rows as an array', () => {
		...
		mockTable.rowManager.findRow.mockReturnValueOnce(mockRows[0]).mockReturnValueOnce(mockRows[2])
	...
	it('should handle rolling selection when max rows is reached', () => {
		...
		mockTable.rowManager.findRow.mockReturnValueOnce(mockRows[0]).mockReturnValueOnce(mockRows[1])
