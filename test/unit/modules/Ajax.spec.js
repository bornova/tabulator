...
describe('Ajax', function () {
	...
	test('requestParams merges parameters correctly', function () {
		...
		const element = document.createElement('div')
		const customTable = new TabulatorFull(element, {
			ajaxURL: 'test.json',
			ajaxParams: { page: 1, size: 10 }
		})

		const customAjax = customTable.module('ajax')
		const result = customAjax.requestParams(null, null, false, { filter: 'active' })
	...
	test('requestParams handles function parameters', function () {
		...
		const customAjax = customTable.module('ajax')
		const result = customAjax.requestParams(null, null, false, { sort: 'name' })
	...
	test('sendRequest handles canceled requests', function (done) {
		...
		customAjax.sendRequest('test.json', {}, {}).catch(() => {
