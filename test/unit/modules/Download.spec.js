...
describe('Download', function () {
	...
	describe('Functionality tests', function () {
		...
		test('download calls downloader function with correct parameters', function () {
			...
			expect(mockTable.modules.export.generateExportList).toHaveBeenCalledWith({}, false, 'active', 'download')

			// Check that downloader was called with correct parameters
			expect(mockDownloader).toHaveBeenCalledWith(mockExportList, options, expect.any(Function))
		})
		...
		test('download accepts a custom function as a downloader', function () {
			...
			expect(customDownloader).toHaveBeenCalledWith(mockExportList, options, expect.any(Function))
		})
		...
		test('generateExportList handles group headers correctly', function () {
			...
			expect(groupHeaderFormatter).toHaveBeenCalledWith('Group 1', 5, { id: 'group1' }, mockComponent)
		...
		test('commsReceived handles intercept action correctly', function () {
			...
			// Check that download was called with the correct parameters
			expect(download.download).toHaveBeenCalledWith('csv', '', { delimiter: ',' }, true, true)
		})
	})
})
