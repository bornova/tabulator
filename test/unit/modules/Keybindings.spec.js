...
describe('Keybindings module', () => {
	...
	beforeEach(() => {
		...
		mockTable = {
			...
			rowManager: {
				...
				scrollToRow: jest.fn(),
				getDisplayRows: jest.fn().mockReturnValue([{ id: 1 }, { id: 2 }, { id: 3 }]),
				displayRowsCount: 3
			},
			columnManager: {
	...
	it('should handle clearing bindings', () => {
		...
		expect(keybindingsMod.table.element.removeEventListener).toHaveBeenCalledWith(
			'keydown',
			keybindingsMod.keyupBinding
		)
		expect(keybindingsMod.table.element.removeEventListener).toHaveBeenCalledWith(
			'keyup',
			keybindingsMod.keydownBinding
		)
	})
	...
	it('should execute navigation actions correctly', () => {
		...
		actions.forEach((action) => {
			...
			expect(keybindingsMod.dispatch).toHaveBeenCalledWith(
				`keybinding-${action.replace('nav', 'nav-').toLowerCase()}`,
				events[action]
			)
		})
	})
