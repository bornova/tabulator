...
test('resizeColumns module', async ({ page }) => {
  ...
  await test.step('_checkResizability and calcGuidePosition enforce resize constraints', async () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const module = table.modules.resizeColumns
      const firstColumn = table.columnManager.columnsByIndex[0]
      const secondColumn = table.columnManager.columnsByIndex[1]

      const checkResizableTrue = module._checkResizability(firstColumn)
      const checkResizableFalse = module._checkResizability(secondColumn)

      module.startX = 100

      const originalColumnBounds = firstColumn.element.getBoundingClientRect
      firstColumn.element.getBoundingClientRect = () => ({ left: 40 })

      const fakeHandle = {
        getBoundingClientRect: () => ({ x: 110 })
      }

      const originalTableBounds = table.element.getBoundingClientRect
      table.element.getBoundingClientRect = () => ({ x: 10 })

      const originalMinWidth = firstColumn.minWidth
      const originalMaxWidth = firstColumn.maxWidth
      firstColumn.minWidth = 60
      firstColumn.maxWidth = 140

      const guideWithinBounds = module.calcGuidePosition({ clientX: 130 }, firstColumn, fakeHandle)
      const guideMinClamped = module.calcGuidePosition({ clientX: 50 }, firstColumn, fakeHandle)
      const guideMaxClamped = module.calcGuidePosition({ clientX: 260 }, firstColumn, fakeHandle)

      firstColumn.element.getBoundingClientRect = originalColumnBounds
      table.element.getBoundingClientRect = originalTableBounds
      firstColumn.minWidth = originalMinWidth
      firstColumn.maxWidth = originalMaxWidth
    ...
    expect(result.checkResizableTrue).toBe(true)
    expect(result.checkResizableFalse).toBe(false)
    expect(result.guideWithinBounds).toBe(130)
    expect(result.guideMinClamped).toBe(90)
    expect(result.guideMaxClamped).toBe(170)
  })
