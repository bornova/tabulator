...
async function checkGroupRowsModuleInternals(page) {
  const { pageErrors, consoleErrors } = attachErrorCollectors(page)
  await page.goto(fixtureUrl)
  ...
  const result = await page.evaluate(async () => {
    ...
    const mod = table.modules.groupRows
    const defaults = {
      groupIDLookupsIsArray: Array.isArray(mod.groupIDLookups),
      startOpenIsArray: Array.isArray(mod.startOpen),
      headerGeneratorIsArray: Array.isArray(mod.headerGenerator),
      groupListIsArray: Array.isArray(mod.groupList),
      displayHandlerIsFunction: typeof mod.displayHandler === 'function',
      blockRedrawInitially: mod.blockRedraw
    }

    mod._blockRedrawing()
    const blockRedrawAfterBlock = mod.blockRedraw
    mod._restoreRedrawing()
    const blockRedrawAfterRestore = mod.blockRedraw

    const groups = table.getGroups()
    const userGroups = groups.map((group) => group.getKey()).sort()

    return {
      defaults,
      blockRedrawAfterBlock,
      blockRedrawAfterRestore,
      userGroups
    }
  })

  expectNoBrowserErrors(pageErrors, consoleErrors)
  expect(result.defaults.groupIDLookupsIsArray).toBe(true)
  expect(result.defaults.startOpenIsArray).toBe(true)
  expect(result.defaults.headerGeneratorIsArray).toBe(true)
  expect(result.defaults.groupListIsArray).toBe(true)
  expect(result.defaults.displayHandlerIsFunction).toBe(true)
  expect(result.defaults.blockRedrawInitially).toBe(false)
  expect(result.blockRedrawAfterBlock).toBe(true)
  expect(result.blockRedrawAfterRestore).toBe(false)
  expect(result.userGroups).toEqual(['A', 'B'])
}
...
test('groupRows module', async ({ page }) => {
  ...
  await test.step('groupRows module internal defaults', async () => {
    await checkGroupRowsModuleInternals(page)
  })
})
