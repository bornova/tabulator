...
test('menu module', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const table = await new Promise((resolve) => {
      const instance = new Tabulator(holder, {
        ...
        groupBy: 'group',
        rowContextMenu: () => makeMenu('rowContextMenu'),
    ...
    const runOption = async (name, trigger) => {
      ...
      const currentComponentSet = !!menuModule.currentComponent
      ...
      const separatorCount = menuEl ? menuEl.querySelectorAll('.tabulator-menu-separator').length : 0
      const menuClassPresent = !!menuEl?.classList.contains('tabulator-menu')
      ...
      await closeMenu()
      const currentComponentCleared = !menuModule.currentComponent

      return {
        opened: !!menuEl,
        currentComponentSet,
        currentComponentCleared,
        menuClassPresent,
        separatorCount,
  ...
  optionKeys.forEach((key) => {
    ...
    expect(result.optionResults[key]?.currentComponentSet, `${key} did not set currentComponent`).toBe(true)
    expect(result.optionResults[key]?.currentComponentCleared, `${key} did not clear currentComponent`).toBe(true)
    expect(result.optionResults[key]?.menuClassPresent, `${key} menu element missing tabulator-menu class`).toBe(true)
    expect(result.optionResults[key]?.separatorCount, `${key} missing separator item`).toBe(1)
