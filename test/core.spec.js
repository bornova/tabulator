...
test('Tabulator core functionality', async ({ page }) => {
  ...
  const fullResult = await page.evaluate(async () => {
    ...
    const initialRowCount = table.getRows('active').length
    const columnCount = table.getColumns().length

    await table.addRow({ id: 6, name: 'Frank', age: 29, gender: 'Male' })
    const rowCountAfterAdd = table.getRows('active').length

    await table.updateRow(1, { name: 'Alice Updated' })
    const updatedName = table.getRow(1).getData().name

    await table.deleteRow(1)
    const rowCountAfterDelete = table.getRows('active').length

    table.setFilter('gender', '=', 'Female')
    const filteredRowCount = table.getRows('active').length

    table.clearFilter()
    const rowCountAfterClearFilter = table.getRows('active').length
    ...
    return {
      ...
      rowCountAfterDelete,
      filteredRowCount,
      rowCountAfterClearFilter
    }
  })

  expect(fullResult.initialRowCount).toBe(5)
  expect(fullResult.columnCount).toBe(4)
  expect(fullResult.rowCountAfterAdd).toBe(6)
  expect(fullResult.updatedName).toBe('Alice Updated')
  expect(fullResult.rowCountAfterDelete).toBe(5)
  expect(fullResult.filteredRowCount).toBe(1)
  expect(fullResult.rowCountAfterClearFilter).toBe(5)
})
