...
test('page module', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    let invalidHighRejected = false
    let invalidLowRejected = false

    try {
      await localTable.setPage(99)
    } catch {
      invalidHighRejected = true
    }

    try {
      await localTable.setPage(0)
    } catch {
      invalidLowRejected = true
    }

    localResult.invalidPageHandling = {
      invalidHighRejected,
      invalidLowRejected,
      pageAfterInvalid: localTable.getPage()
    }
  ...
  expect(result.localResult.invalidPageHandling).toEqual({
    invalidHighRejected: true,
    invalidLowRejected: true,
    pageAfterInvalid: 1
  })
