...
test('edit module', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const setValueCell = row.getCell('inputField')
    const setValueBefore = setValueCell.getValue()
    let setValueEventMatched = false
    const onCellEdited = (cell) => {
      if (cell.getField() === 'inputField' && cell.getValue() === 'Updated Name') {
        setValueEventMatched = true
      }
    }
    table.on('cellEdited', onCellEdited)
    setValueCell.setValue('Updated Name')
    const setValueAfter = setValueCell.getValue()
    const rowDataAfterSetValue = row.getData().inputField

    const editModule = table.modules.edit
    const editorCountBefore = Object.keys(editModule.editors).length
    const customEditor = () => document.createElement('input')
    editModule.editors.custom = customEditor
    const editorCountAfter = Object.keys(editModule.editors).length
    const customEditorRegistered = editModule.editors.custom === customEditor
    ...
    const zipLookupCell = row2.getCell('zipLookupField')
    const zipLookupBefore = zipLookupCell.getValue()

    zipLookupCell.edit()
    const zipLookupInput = zipLookupCell.getElement().querySelector('input[data-editor="list-lookup"]')
    zipLookupInput.dispatchEvent(new Event('blur', { bubbles: true }))
    ...
    return {
      ...
      setValueBefore,
      setValueAfter,
      rowDataAfterSetValue,
      setValueEventMatched,
      editorCountBefore,
      editorCountAfter,
      customEditorRegistered,
  ...
  expect(result.setValueBefore).toBe('alpha')
  expect(result.setValueAfter).toBe('Updated Name')
  expect(result.rowDataAfterSetValue).toBe('Updated Name')
  expect(result.setValueEventMatched).toBe(true)
  expect(result.editorCountAfter).toBe(result.editorCountBefore + 1)
  expect(result.customEditorRegistered).toBe(true)
