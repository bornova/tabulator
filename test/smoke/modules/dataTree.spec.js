...
async function checkDataTreeModuleApi(page) {
  const { pageErrors, consoleErrors } = attachErrorCollectors(page)
  await page.goto(fixtureUrl)
  ...
  const result = await page.evaluate(async () => {
    ...
    const mod = table.modules.dataTree
    const rootRow = table.getRows()[0]._row

    const treeChildrenRows = mod.getTreeChildren(rootRow, false, false)
    const treeChildrenComponents = mod.getTreeChildren(rootRow, true, false)
    const parentComponent = mod.getTreeParent(treeChildrenRows[0])

    const isExpandedInitially = mod.isRowExpanded(rootRow)
    mod.expandRow(rootRow)
    await new Promise((resolve) => setTimeout(resolve, 25))
    const isExpandedAfterExpand = mod.isRowExpanded(rootRow)

    mod.toggleRow(rootRow)
    await new Promise((resolve) => setTimeout(resolve, 25))
    const isExpandedAfterToggle = mod.isRowExpanded(rootRow)

    mod.collapseRow(rootRow)
    await new Promise((resolve) => setTimeout(resolve, 25))
    const isExpandedAfterCollapse = mod.isRowExpanded(rootRow)

    mod.addTreeChildRow(rootRow, { id: 99, name: 'Added First' }, true)
    mod.addTreeChildRow(rootRow, { id: 100, name: 'Added Last' }, false)

    const childrenData = rootRow.getData()._children

    return {
      treeChildrenRowCount: treeChildrenRows.length,
      treeChildrenComponentCount: treeChildrenComponents.length,
      hasParentComponent: !!parentComponent,
      isExpandedInitially,
      isExpandedAfterExpand,
      isExpandedAfterToggle,
      isExpandedAfterCollapse,
      childCountAfterAdd: childrenData.length,
      firstChildNameAfterAdd: childrenData[0].name,
      lastChildNameAfterAdd: childrenData[childrenData.length - 1].name
    }
  })

  expectNoBrowserErrors(pageErrors, consoleErrors)
  expect(result.treeChildrenRowCount).toBe(2)
  expect(result.treeChildrenComponentCount).toBe(2)
  expect(result.hasParentComponent).toBe(true)
  expect(result.isExpandedInitially).toBe(false)
  expect(result.isExpandedAfterExpand).toBe(true)
  expect(result.isExpandedAfterToggle).toBe(false)
  expect(result.isExpandedAfterCollapse).toBe(false)
  expect(result.childCountAfterAdd).toBe(4)
  expect(result.firstChildNameAfterAdd).toBe('Added First')
  expect(result.lastChildNameAfterAdd).toBe('Added Last')
}
...
test('dataTree module', async ({ page }) => {
  ...
  await test.step('dataTree module API methods', async () => {
    await checkDataTreeModuleApi(page)
  })
})
