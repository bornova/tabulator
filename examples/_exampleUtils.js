const OMNITABLE_SOURCE_STORAGE_KEY = 'tabulator-example-source'
const OMNITABLE_SOURCE_QUERY_KEY = 'tabulator-source'
const OMNITABLE_STYLESHEET_ID = 'tabulator-example-stylesheet'
const OMNITABLE_BROWSER_SCRIPT_ID = 'tabulator-example-browser-script'
const OMNITABLE_CDN_SCRIPT_ID = 'tabulator-example-cdn-script'
...
export const initializeTabulatorSourceSelector = (initialSource = getSelectedTabulatorSource()) => {
  ...
  sourceToggle.addEventListener('click', (event) => {
    ...
    if (nextSource && nextSource !== getSelectedTabulatorSource()) {
      localStorage.setItem(OMNITABLE_SOURCE_STORAGE_KEY, nextSource)

      const url = new URL(window.location.href)
      url.searchParams.set(OMNITABLE_SOURCE_QUERY_KEY, nextSource)
...
const getSelectedTabulatorSource = () => {
  ...
  const sourceFromQuery = params.get(OMNITABLE_SOURCE_QUERY_KEY)

  if (sourceFromQuery && tabulatorSources[sourceFromQuery]) {
    localStorage.setItem(OMNITABLE_SOURCE_STORAGE_KEY, sourceFromQuery)
    return sourceFromQuery
  }

  const storedSource = localStorage.getItem(OMNITABLE_SOURCE_STORAGE_KEY)
...
const ensureTabulatorStylesheet = (source) => {
  ...
  return new Promise((resolve) => {
    let stylesheet = document.getElementById(OMNITABLE_STYLESHEET_ID)

    if (!stylesheet) {
      stylesheet = document.createElement('link')
      stylesheet.id = OMNITABLE_STYLESHEET_ID
...
const ensureBrowserTabulatorScript = () => {
  ...
  return ensureTabulatorScript({
    id: OMNITABLE_BROWSER_SCRIPT_ID,
