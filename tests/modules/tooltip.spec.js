...
test('tooltip module', async ({ page }) => {
  ...
  await test.step('function tooltip receives component and renders returned content', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      const module = table.modules.tooltip
      const cell = table.getRows()[0].getCell('age')._cell
      const event = new MouseEvent('mousemove', { clientX: 25, clientY: 25 })

      module.loadTooltip(event, cell, cell.column.definition.tooltip)
  ...
  await test.step('mouseout clears pending tooltip timeout when popup is not active', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      module.mousemoveCheck('tooltip', event, cell)
      const hadTimeoutAfterMove = module.timeout !== null

      module.mouseoutCheck('tooltip', event, cell)
      const timeoutAfterMouseout = module.timeout

      await new Promise((resolve) => setTimeout(resolve, 120))

      return {
        hadTimeoutAfterMove,
        timeoutCleared: timeoutAfterMouseout === null,
        popupStillNull: module.popupInstance === null
      }
    })

    expect(result.hadTimeoutAfterMove).toBe(true)
    expect(result.timeoutCleared).toBe(true)
    expect(result.popupStillNull).toBe(true)
  })
