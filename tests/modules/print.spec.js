...
test('print module', async ({ page }) => {
  ...
  await test.step('destroy removes print event handlers when printAsHtml is enabled', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      window.removeEventListener = function (type, handler, options) {
        removed.push({ type, matchesHandler: typeof handler === 'function' })
      ...
      table.modules.print.destroy()
      window.removeEventListener = originalRemove

      return {
        removedBefore: removed.some((entry) => entry.type === 'beforeprint' && entry.matchesHandler),
        removedAfter: removed.some((entry) => entry.type === 'afterprint' && entry.matchesHandler)
      }
    })

    expect(result.removedBefore).toBe(true)
    expect(result.removedAfter).toBe(true)
  })
  ...
  await test.step('replaceTable does nothing when manualBlock is active', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      const mod = table.modules.print
      let generateCalls = 0
      const originalGenerate = table.modules.export.generateTable
      table.modules.export.generateTable = (...args) => {
        generateCalls += 1
        return originalGenerate(...args)
      }

      mod.manualBlock = true
      mod.replaceTable()

      const printTables = holder.querySelectorAll('.tabulator-print-table').length

      return {
        generateCalls,
        printTables,
        elementStillFalse: mod.element === false
      }
    })

    expect(result.generateCalls).toBe(0)
    expect(result.printTables).toBe(0)
    expect(result.elementStillFalse).toBe(true)
  })

  await test.step('printHeader and printFooter functions can return elements for print sections', async () => {
    await page.goto(fixtureUrl)
      ...
      const table = await new Promise((resolve) => {
        const instance = new Tabulator(holder, {
          printHeader() {
            ...
            el.textContent = 'HEADER_NODE'
            return el
          },
          printFooter() {
            footerCalled = true
            const el = document.createElement('div')
            el.textContent = 'FOOTER_NODE'
            return el
          },
          printFormatter(el) {
            headerText = el.querySelector('.tabulator-print-header h1')?.textContent || null
            footerText = el.querySelector('.tabulator-print-footer div')?.textContent || null
          },
          data: [{ id: 1, name: 'alice' }],
          columns: [
            { title: 'ID', field: 'id' },
    ...
    expect(result.headerCalled).toBe(true)
    expect(result.footerCalled).toBe(true)
    expect(result.headerText).toBe('HEADER_NODE')
    expect(result.footerText).toBe('FOOTER_NODE')
  })
