...
async function checkHtmlImportNonTableAndEmptyWarnings(page) {
  const { pageErrors, consoleErrors } = attachErrorCollectors(page)
  await page.goto(fixtureUrl)
  ...
  const result = await page.evaluate(async () => {
    ...
    divEl.id = 'html-import-div'
    root.appendChild(divEl)

    const emptyTableEl = document.createElement('table')
    emptyTableEl.id = 'html-import-empty-table'
    root.appendChild(emptyTableEl)
    ...
    const divInstance = await new Promise((resolve) => {
      const instance = new Tabulator(divEl, {})
    ...
    const emptyTableInstance = await new Promise((resolve) => {
      const instance = new Tabulator(emptyTableEl, {})
    ...
    const htmlImportingEventCount = warnings.filter((warning) => warning.includes('htmlImporting')).length
    const htmlImportedEventCount = warnings.filter((warning) => warning.includes('htmlImported')).length

    return {
      divDataLength: divInstance.getData().length,
      emptyTableDataLength: emptyTableInstance.getData().length,
      emptyTableWarning: warnings.some((warning) =>
        warning.includes(
      ...
      htmlImportingEventCount,
      htmlImportedEventCount,
      attribTrue: divInstance.modules.htmlTableImport._attribValue('true'),
      attribFalse: divInstance.modules.htmlTableImport._attribValue('false'),
      attribNumber: divInstance.modules.htmlTableImport._attribValue('123'),
      attribString: divInstance.modules.htmlTableImport._attribValue('string')
    }
  })

  expectNoBrowserErrors(pageErrors, consoleErrors)
  expect(result.divDataLength).toBe(0)
  expect(result.emptyTableDataLength).toBe(0)
  expect(result.emptyTableWarning).toBe(true)
  expect(result.htmlImportingEventCount).toBe(0)
  expect(result.htmlImportedEventCount).toBe(0)
  expect(result.attribTrue).toBe(true)
  expect(result.attribFalse).toBe(false)
  expect(result.attribNumber).toBe('123')
  expect(result.attribString).toBe('string')
}
...
test('htmlTableImport module', async ({ page }) => {
  ...
  await test.step('non-table and empty-table edge cases', async () => {
    await checkHtmlImportNonTableAndEmptyWarnings(page)
  })
})
