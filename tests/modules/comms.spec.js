...
test('comms module', async ({ page }) => {
  ...
  await test.step('comms module is initialized with table reference', async () => {
    const initialized = await page.evaluate(() => {
      const comms = window.tab1.modules.comms
      return !!comms && comms.table === window.tab1
    })
  ...
  await test.step('receive warns when target module does not exist', async () => {
    const warned = await page.evaluate(() => {
      ...
      comms.receive(window.tab2, 'nonexistentModule', 'testAction', { test: 'data' })
