...
test('mutator module', async ({ page }) => {
  ...
  await test.step('mutateLink supports array links and retriggers each linked cell', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      const table = await new Promise((resolve) => {
        const instance = new Tabulator(holder, {
          data: [{ id: 1, source: 10, targetA: 20, targetB: 30 }],
          columns: [
            { title: 'ID', field: 'id' },
            { title: 'Source', field: 'source', mutateLink: ['targetA', 'targetB'] },
            { title: 'Target A', field: 'targetA' },
            { title: 'Target B', field: 'targetB' }
          ]
        })
      ...
      const row = table.rowManager.getRows()[0]
      const sourceCell = row.getCell('source')
      const targetA = row.getCell('targetA')
      const targetB = row.getCell('targetB')
      const calls = []

      targetA.setValue = (...args) => {
        calls.push({ field: 'targetA', args })
      }
      targetB.setValue = (...args) => {
        calls.push({ field: 'targetB', args })
      }
      ...
      return {
        callCount: calls.length,
        fields: calls.map((entry) => entry.field).sort(),
        allFlagsTrue: calls.every((entry) => entry.args[1] === true && entry.args[2] === true)
      }
    })

    expect(result.callCount).toBe(2)
    expect(result.fields).toEqual(['targetA', 'targetB'])
    expect(result.allFlagsTrue).toBe(true)
  })

  await test.step('invalid mutator name warns and is ignored', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      const table = await new Promise((resolve) => {
        const instance = new Tabulator(holder, {
          ...
          columns: [
            { title: 'ID', field: 'id' },
            { title: 'Bad', field: 'bad', mutator: 'definitelyMissingMutator' }
          ]
        })
      ...
      const row = table.rowManager.getRows()[0]
      const value = row.getData().bad

      return {
        warningSeen: warnings.some(
          (entry) =>
            entry.includes('Mutator Error - No such mutator found, ignoring:') &&
            entry.includes('definitelyMissingMutator')
        ),
        value
      }
    })
  ...
  await test.step('disable and enable toggle row mutation application', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      const mod = table.modules.mutator

      mod.disable()
      const disabledTransform = mod.transformRow({ name: 'john doe' }, 'data')

      mod.enable()
      const enabledTransform = mod.transformRow({ name: 'john doe' }, 'data')

      return {
        disabledName: disabledTransform.name,
        enabledName: enabledTransform.name,
        enabledState: mod.enabled
      }
    })

    expect(result.disabledName).toBe('john doe')
    expect(result.enabledName).toBe('JOHN DOE')
