...
test('format module', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const sanitized = formatModule.sanitizeHTML("<script>alert('test')</script><b>Hello</b>")

    const emptyToSpaceResults = {
      nullValue: formatModule.emptyToSpace(null),
      undefinedValue: formatModule.emptyToSpace(undefined),
      emptyString: formatModule.emptyToSpace(''),
      textValue: formatModule.emptyToSpace('test'),
      zeroValue: formatModule.emptyToSpace(0)
    }

    const plaintextLookup = formatModule.lookupFormatter('plaintext')
    const directFormatter = (cell) => cell
    const returnedDirectFormatter = formatModule.lookupFormatter(directFormatter)

    const originalWarn = console.warn
    let invalidLookupWarned = false
    console.warn = () => {
      invalidLookupWarned = true
    }
    const invalidLookup = formatModule.lookupFormatter('nonExistentFormatter')
    console.warn = originalWarn

    const typeFormatterColumn = {
      definition: {
        formatter: 'money',
    ...
    const defaultTypeFormatter = formatModule.lookupTypeFormatter(typeFormatterColumn, '')
    const printTypeFormatter = formatModule.lookupTypeFormatter(typeFormatterColumn, 'Print')
    ...
    return {
      ...
      sanitized,
      emptyToSpaceResults,
      plaintextLookupIsFunction: typeof plaintextLookup === 'function',
      directLookupIsSameFunction: returnedDirectFormatter === directFormatter,
      invalidLookupWarned,
      invalidLookupIsFunction: typeof invalidLookup === 'function',
      defaultTypeFormatterHasFormatter: typeof defaultTypeFormatter.formatter === 'function',
      defaultTypeFormatterParams: defaultTypeFormatter.params,
      printTypeFormatterHasFormatter: typeof printTypeFormatter.formatter === 'function',
      printTypeFormatterParams: printTypeFormatter.params
    }
  })
  ...
  expect(result.emptyToSpaceResults.nullValue).toBe('&nbsp;')
  expect(result.emptyToSpaceResults.undefinedValue).toBe('&nbsp;')
  expect(result.emptyToSpaceResults.emptyString).toBe('&nbsp;')
  expect(result.emptyToSpaceResults.textValue).toBe('test')
  expect(result.emptyToSpaceResults.zeroValue).toBe(0)
  expect(result.plaintextLookupIsFunction).toBe(true)
  expect(result.directLookupIsSameFunction).toBe(true)
  expect(result.invalidLookupWarned).toBe(true)
  expect(result.invalidLookupIsFunction).toBe(true)
  expect(result.defaultTypeFormatterHasFormatter).toBe(true)
  expect(result.defaultTypeFormatterParams).toEqual({ symbol: '$' })
