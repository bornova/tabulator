...
test('resizeRows module', async ({ page }) => {
  ...
  await test.step('resize and calcGuidePosition compute expected values', async () => {
    await page.goto(fixtureUrl)
    ...
    const result = await page.evaluate(async () => {
      ...
      const mod = table.modules.resizeRows
      const row = table.rowManager.getRows()[0]
      ...
      mod.resize({ screenY: 100 }, mockRow)

      mod.startY = 150

      const originalTableBounds = table.element.getBoundingClientRect
      const originalRowBounds = row.element.getBoundingClientRect

      table.element.getBoundingClientRect = () => ({ y: 50 })
      row.element.getBoundingClientRect = () => ({ top: 100 })

      const fakeHandle = {
        getBoundingClientRect: () => ({ y: 120 })
      }

      const guidePosition = mod.calcGuidePosition({ screenY: 180 }, row, fakeHandle)

      table.element.getBoundingClientRect = originalTableBounds
      row.element.getBoundingClientRect = originalRowBounds
    ...
    expect(result.resizedHeight).toBe(50)
    expect(result.guidePosition).toBe(100)
  })
