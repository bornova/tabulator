...
test('popup module', async ({ page }) => {
  const { pageErrors, consoleErrors } = attachErrorCollectors(page)
  await page.goto(fixtureUrl)

  await test.step('headerPopup uses default icon when headerPopupIcon is not provided', async () => {
    const result = await page.evaluate(async () => {
        ...
        const instance = new Tabulator(holder, {
          ...
          columns: [
            { title: 'ID', field: 'id' },
            {
              title: 'Name',
              field: 'name',
              headerPopup: () => 'DEFAULT_ICON_HEADER_POPUP'
            }
          ]
        })
      ...
      const button = holder.querySelector('.tabulator-header-popup-button')

      return {
        hasButton: !!button,
        buttonHtml: button?.innerHTML || '',
        buttonText: (button?.textContent || '').trim(),
        modulePresent: !!table.modules.popup
      }
    })

    expect(result.modulePresent).toBe(true)
    expect(result.hasButton).toBe(true)
    expect(result.buttonHtml.length).toBeGreaterThan(0)
  ...
  const result = await page.evaluate(async () => {
    ...
    popupModule.dispatch('cell-dblclick', fakeEvent, cell)
  ...
  await test.step('component popup API forwards content and custom position', async () => {
    const result = await page.evaluate(async () => {
      const root = document.getElementById('smoke-root')
      const holder = document.createElement('div')
      holder.style.width = '600px'
      ...
      popupModule.loadPopup = (e, component, contents, renderedCallback, position) => {
        calls.push({
          hasEvent: !!e,
          contents: String(contents),
          position,
          hasComponent: !!component,
          hasRenderedCallback: typeof renderedCallback === 'function'
        })
      }

      const rowComponent = table.getRow(1)
      rowComponent.popup('COMPONENT_POPUP_CONTENT', 'bottom')
    ...
    expect(result.firstCall).toEqual({
      hasEvent: false,
      contents: 'COMPONENT_POPUP_CONTENT',
      position: 'bottom',
      hasComponent: true,
