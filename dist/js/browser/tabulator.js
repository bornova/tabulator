...
var Tabulator = function() {
    ...
    class Cell extends CoreFeature {
        ...
        _generateContents() {
            let val;
            val = this.chain("cell-format", this, null, () => this.element.innerHTML = this.value);
            switch (typeof val) {
              case "object":
    ...
    class Column extends CoreFeature {
        ...
        verticalAlign(alignment, height) {
            const parentHeight = this.parent.isGroup ? this.parent.getGroupElement().clientHeight : height || this.parent.getHeadersElement().clientHeight;
    ...
    class ColumnManager extends CoreFeature {
        ...
        getColumnsByFieldRoot(root) {
            ...
            Object.keys(this.columnsByField).forEach(field => {
                const fieldRoot = this.table.options.nestedFieldSeparator ? field.split(this.table.options.nestedFieldSeparator)[0] : field;
                if (fieldRoot === root) {
        ...
        getFlexBaseWidth() {
            let totalWidth = this.table.element.clientWidth;
            let fixedWidth = 0;
    ...
    class RowManager extends CoreFeature {
        ...
        getRowFromPosition(position) {
            return this.getDisplayRows().find(row => row.type === "row" && row.getPosition() === position && row.isDisplayed());
        }
            ...
            switch (stage) {
              ...
              case "displayPipeline":
                for (let i = index; i < this.displayPipeline.length; i++) {
                    const result = this.displayPipeline[i].handler((i ? this.getDisplayRows(i - 1) : this.activeRows).slice(0), renderInPosition);
        ...
        adjustTableSize() {
            ...
            if (this.renderer.verticalFillMode === "fill") {
                const otherHeight = Math.floor(this.table.columnManager.getElement().getBoundingClientRect().height + (this.table.footerManager && this.table.footerManager.active && !this.table.footerManager.external ? this.table.footerManager.getElement().getBoundingClientRect().height : 0));
                if (this.fixedHeight) {
    ...
    class DataLoader extends CoreFeature {
        ...
        alertLoader() {
            const shouldLoad = typeof this.table.options.dataLoader === "function" ? this.table.options.dataLoader() : this.table.options.dataLoader;
            if (shouldLoad) {
    ...
    class Module extends CoreFeature {
        ...
        displayRows(adjust) {
            ...
            let lookupIndex;
            if (this._handler) {
                lookupIndex = this.table.rowManager.displayPipeline.findIndex(item => item.handler === this._handler);
                if (lookupIndex > -1) {
                    index = lookupIndex;
                }
            }
    ...
    function fitColumns(columns, forced) {
        let totalWidth = this.table.rowManager.element.getBoundingClientRect().width;
        let fixedWidth = 0;
        let flexWidth = 0;
        let flexGrowUnits = 0;
        let flexColWidth = 0;
        const flexColumns = [];
        const fixedShrinkColumns = [];
        let flexShrinkUnits = 0;
        let overflowWidth = 0;
        let gapFill = 0;
    ...
    class Layout extends Module {
        ...
        layout(dataChanged) {
            const variableHeight = this.table.columnManager.columnsByIndex.find(column => column.definition.variableHeight || column.definition.formatter === "textarea");
    ...
    class Tabulator extends ModuleBinder {
        ...
        addRow(data, pos, index) {
            ...
            return this.rowManager.addRows(data, pos, index, true).then(rows => rows[0].getComponent());
        }
        updateOrAddRow(index, data) {
            ...
            if (row) {
                return row.updateData(data).then(() => row.getComponent());
            } else {
                return this.rowManager.addRows(data).then(rows => rows[0].getComponent());
            }
        }
        updateRow(index, data) {
            ...
            if (row) {
                return row.updateData(data).then(() => Promise.resolve(row.getComponent()));
            } else {
        ...
        addColumn(definition, before, field) {
            const column = this.columnManager.findColumn(field);
            this.initGuard();
            return this.columnManager.addColumn(definition, before, column).then(column => column.getComponent());
        }
        deleteColumn(field) {
            const column = this.columnManager.findColumn(field);
            this.initGuard();
    ...
    function serializeParams(params) {
        const output = generateParamsList$1(params);
        const encoded = [];
    ...
    var defaultCalculations = {
        ...
        unique: function(values, data, calcParams) {
            const unique = values.filter((value, index) => (values || value === 0) && values.indexOf(value) === index);
            ...
            if (typeof subject === "object") {
                if (subject instanceof Row) {
                    ...
                    if (parent.modules.dataTree) {
                        match = parent.modules.dataTree.children.find(childRow => childRow instanceof Row ? childRow.element === subject : false);
                        if (match) {
                            match = match.data;
                        }
                    }
    ...
    class Download extends Module {
        ...
        generateExportList(range) {
            const list = this.table.modules.export.generateExportList(this.table.options.downloadConfig, false, range || this.table.options.downloadRowRange, "download");
    ...
    let Edit$1 = class Edit {
        ...
        _scrollToValue(char) {
            ...
            const match = this.displayItems.find(item => typeof item.label !== "undefined" && item.label.toLowerCase().startsWith(this.filterTerm));
            if (match) {
        ...
        _ajaxRequest(url, term) {
            const params = this.params.filterRemote ? {
                term: term
            } : {};
    ...
    function list(cell, onRendered, success, cancel, editorParams) {
        const list = new Edit$1(this, cell, onRendered, success, cancel, editorParams);
        return list.input;
    }
    function star$1(cell, onRendered, success, cancel, editorParams) {
        ...
        const size = element.getElementsByTagName("svg")[0] ? element.getElementsByTagName("svg")[0].getAttribute("width") : 14;
        const stars = [];
    ...
    function progress$1(cell, onRendered, success, cancel, editorParams) {
        const element = cell.getElement();
        const max = typeof editorParams.max === "undefined" ? element.getElementsByTagName("div")[0] && element.getElementsByTagName("div")[0].getAttribute("max") || 100 : editorParams.max;
        const min = typeof editorParams.min === "undefined" ? element.getElementsByTagName("div")[0] && element.getElementsByTagName("div")[0].getAttribute("min") || 0 : editorParams.min;
        ...
        function updateValue() {
            ...
            const calcVal = percent * Math.round(bar.offsetWidth / ((element.clientWidth - parseInt(style.getPropertyValue("padding-left")) - parseInt(style.getPropertyValue("padding-right"))) / 100)) + min;
            success(calcVal);
    ...
    class Export extends Module {
        ...
        generateHeaderElement(row, setup, styles) {
            ...
            row.columns.forEach(column => {
                if (column) {
                    ...
                    const classNames = column.component._column.definition.cssClass ? column.component._column.definition.cssClass.split(" ") : [];
        ...
        getHtml(visible, style, config, colVisProp) {
            const list = this.generateExportList(config || this.table.options.htmlOutputConfig, style, visible, colVisProp || "htmlOutput");
    ...
    class Filter extends Module {
        ...
        initializeColumn(column, value) {
            ...
            const field = column.getField();
            function success(value) {
                const filterType = column.modules.filter.tagType == "input" && column.modules.filter.attrType == "text" || column.modules.filter.tagType == "textarea" ? "partial" : "match";
                let type = "";
        ...
        removeFilter(field, type, value) {
            ...
            field.forEach(filter => {
                ...
                if (typeof filter.field === "object") {
                    index = this.filterList.findIndex(element => filter === element);
                } else {
                    index = this.filterList.findIndex(element => filter.field === element.field && filter.type === element.type && filter.value === element.value);
                }
                if (index > -1) {
    ...
    function tickCross(cell, formatterParams, onRendered) {
        ...
        const tick = typeof formatterParams.tickElement !== "undefined" ? formatterParams.tickElement : '<svg enable-background="new 0 0 24 24" height="14" width="14" viewBox="0 0 24 24" xml:space="preserve" ><path fill="#2DC214" clip-rule="evenodd" d="M21.652,3.211c-0.293-0.295-0.77-0.295-1.061,0L9.41,14.34  c-0.293,0.297-0.771,0.297-1.062,0L3.449,9.351C3.304,9.203,3.114,9.13,2.923,9.129C2.73,9.128,2.534,9.201,2.387,9.351  l-2.165,1.946C0.078,11.445,0,11.63,0,11.823c0,0.194,0.078,0.397,0.223,0.544l4.94,5.184c0.292,0.296,0.771,0.776,1.062,1.07  l2.124,2.141c0.292,0.293,0.769,0.293,1.062,0l14.366-14.34c0.293-0.294,0.293-0.777,0-1.071L21.652,3.211z" fill-rule="evenodd"/></svg>';
        const cross = typeof formatterParams.crossElement !== "undefined" ? formatterParams.crossElement : '<svg enable-background="new 0 0 24 24" height="14" width="14"  viewBox="0 0 24 24" xml:space="preserve" ><path fill="#CE1515" d="M22.245,4.015c0.313,0.313,0.313,0.826,0,1.139l-6.276,6.27c-0.313,0.312-0.313,0.826,0,1.14l6.273,6.272  c0.313,0.313,0.313,0.826,0,1.14l-2.285,2.277c-0.314,0.312-0.828,0.312-1.142,0l-6.271-6.271c-0.313-0.313-0.828-0.313-1.141,0  l-6.276,6.267c-0.313,0.313-0.828,0.313-1.141,0l-2.282-2.28c-0.313-0.313-0.313-0.826,0-1.14l6.278-6.269  c0.313-0.312,0.313-0.826,0-1.14L1.709,5.147c-0.314-0.313-0.314-0.827,0-1.14l2.284-2.278C4.308,1.417,4.821,1.417,5.135,1.73  L11.405,8c0.314,0.314,0.828,0.314,1.141,0.001l6.276-6.267c0.312-0.312,0.826-0.312,1.141,0L22.245,4.015z"/></svg>';
    ...
    function traffic(cell, formatterParams, onRendered) {
        ...
        const colors = formatterParams && typeof formatterParams.color !== "undefined" ? formatterParams.color : [ "red", "orange", "green" ];
        let color = "#666666";
        let percent;
        let percentValue;
        ...
        if (formatterParams.valueMap) {
            if (typeof formatterParams.valueMap === "string") {
                valueMap = function(value) {
                    return value.map(item => Helpers.retrieveNestedData(table.options.nestedFieldSeparator, formatterParams.valueMap, item));
                };
            } else {
    ...
    class Format extends Module {
        ...
        formatValue(cell) {
            ...
            const params = typeof cell.column.modules.format.params === "function" ? cell.column.modules.format.params(component) : cell.column.modules.format.params;
            function onRendered(callback) {
    ...
    class Group {
        ...
        _createGroup(groupID, level) {
            ...
            const group = new Group(this.groupManager, this, level, groupID, this.groupManager.groupIDLookups[level].field, this.groupManager.headerGenerator[level] || this.groupManager.headerGenerator[0], this.old ? this.old.groups[groupKey] : false);
    ...
    class GroupRows extends Module {
        ...
        configureGroupSetup() {
            if (this.table.options.groupBy) {
                ...
                this.langBind("groups|item", (langValue, lang) => {
                    this.headerGenerator[0] = (value, count, data) => (typeof value === "undefined" ? "" : value) + "<span>(" + count + " " + (count === 1 ? langValue : lang.groups.items) + ")</span>";
                });
                this.groupIDLookups = [];
                if (groupBy) {
        ...
        reassignRowToGroup(row) {
            if (row.type === "row") {
                ...
                let samePath;
                samePath = oldGroupPath.length == newGroupPath.length && oldGroupPath.every((element, index) => element === newGroupPath[index]);
                if (!samePath) {
    ...
    class HtmlTableImport extends Module {
        ...
        _findCol(title) {
            const match = this.table.options.columns.find(column => column.title === title);
            return match || false;
        }
    ...
    class Page extends Module {
        ...
        _setPageButtons() {
            ...
            const min = this.max - this.page + leftSize + 1 < this.count ? this.max - this.count + 1 : Math.max(this.page - leftSize, 1);
    ...
    class Print extends Module {
        ...
        printFullscreen(visible, style, config) {
            ...
            const tableEl = this.table.modules.export.generateTable(typeof config !== "undefined" ? config : this.table.options.printConfig, typeof style !== "undefined" ? style : this.table.options.printStyled, visible || this.table.options.printRowRange, "print");
            let headerContent;
            let footerContent;
    ...
    class SelectRange extends Module {
        ...
        constructor(table) {
            ...
            this.registerTableOption("selectableRangeClearCellsValue", undefined);
        ...
        findJumpCellLeft(rowPos, colPos) {
            ...
            const isLeftOfStartingCellEmpty = columns[colPos - 1] ? this.isEmpty(row.getData()[columns[colPos - 1].getField()]) : false;
        ...
        findJumpCellRight(rowPos, colPos) {
            ...
            const columns = this.getTableColumns();
            const isStartingCellEmpty = this.isEmpty(row.getData()[columns[colPos].getField()]);
            const isRightOfStartingCellEmpty = columns[colPos + 1] ? this.isEmpty(row.getData()[columns[colPos + 1].getField()]) : false;
            const jumpCol = this.findJumpCol(row, columns.slice(colPos + 1, columns.length), false, isStartingCellEmpty, isRightOfStartingCellEmpty);
            if (jumpCol) {
                return jumpCol.getPosition() - 1;
            }
            return colPos;
        ...
        findJumpCellUp(rowPos, colPos) {
            ...
            const isTopOfStartingCellEmpty = rows[rowPos - 1] ? this.isEmpty(rows[rowPos - 1].getData()[column.getField()]) : false;
        ...
        findJumpCellDown(rowPos, colPos) {
            ...
            const rows = this.getTableRows();
            const isStartingCellEmpty = this.isEmpty(rows[rowPos].getData()[column.getField()]);
            const isBottomOfStartingCellEmpty = rows[rowPos + 1] ? this.isEmpty(rows[rowPos + 1].getData()[column.getField()]) : false;
            const jumpRow = this.findJumpRow(column, rows.slice(rowPos + 1, rows.length), false, isStartingCellEmpty, isBottomOfStartingCellEmpty);
            if (jumpRow) {
                return jumpRow.position - 1;
            }
            return rowPos;
    ...
    function array(a, b, aRow, bRow, column, dir, params) {
        ...
        if (params.valueMap) {
            if (typeof params.valueMap === "string") {
                valueMap = function(value) {
                    return value.map(item => Helpers.retrieveNestedData(table.options.nestedFieldSeparator, params.valueMap, item));
                };
            } else {
    ...
    class Validate extends Module {
        ...
        editValidate(cell, value, previousValue) {
            const valid = this.table.options.validationMode !== "manual" ? this.validate(cell.column.modules.validate, cell, value) : true;
            if (valid !== true) {
                setTimeout(() => {
