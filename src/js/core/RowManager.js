...
export default class RowManager extends CoreFeature {
  ...
  constructor(table) {
    ...
    // Lazy O(1) row→index caches; invalidated on structural changes.
    this._rowIndexCache = null // for this.rows
    this._activeRowIndexCache = null // for this.activeRows
  }

  /**
   * Look up index of a row in this.rows in O(1) using a lazy cache.
   * @param {Row} row Row instance.
   * @returns {number} Index or -1.
   */
  _indexOfRow(row) {
    if (!this._rowIndexCache) {
      const cache = new Map()
      for (let i = 0; i < this.rows.length; i++) cache.set(this.rows[i], i)
      this._rowIndexCache = cache
    }
    const idx = this._rowIndexCache.get(row)
    return idx === undefined ? -1 : idx
  }

  /**
   * Look up index of a row in this.activeRows in O(1) using a lazy cache.
   * @param {Row} row Row instance.
   * @returns {number} Index or -1.
   */
  _indexOfActiveRow(row) {
    if (!this._activeRowIndexCache) {
      const cache = new Map()
      for (let i = 0; i < this.activeRows.length; i++) cache.set(this.activeRows[i], i)
      this._activeRowIndexCache = cache
    }
    const idx = this._activeRowIndexCache.get(row)
    return idx === undefined ? -1 : idx
  }
  ...
  destroy() {
    ...
    this._invalidateRowIndexCaches()
  }
  ...
  deleteRow(row, blockRedraw) {
    const allIndex = this._indexOfRow(row)
    const activeIndex = this._indexOfActiveRow(row)
  ...
  addRowActual(data, pos, index, blockRedraw) {
    ...
    if (index) {
      allIndex = this._indexOfRow(index)
    }

    if (index && allIndex > -1) {
      activeIndex = this._indexOfActiveRow(index)
  ...
  moveRowInArray(rows, from, to, after) {
    ...
    if (from !== to) {
      ...
      if (rows === this.rows) this._rowIndexCache = null
      if (rows === this.activeRows) this._activeRowIndexCache = null
  ...
  nextDisplayRow(row, rowOnly) {
    ...
    if (rowOnly && nextRow && (!(nextRow instanceof Row) || nextRow.type !== 'row')) {
  ...
  findRowIndex(row, list) {
    ...
    if (row) {
      if (list === this.rows) {
        rowIndex = this._indexOfRow(row)
      } else if (list === this.activeRows) {
        rowIndex = this._indexOfActiveRow(row)
      } else {
