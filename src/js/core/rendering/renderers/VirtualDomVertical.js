...
export default class VirtualDomVertical extends Renderer {
  ...
  rerenderRows(callback) {
    ...
    // Snapshot the anchor as a Row/Group instance (not a numeric index): the
    // display rows array can change inside `callback` (e.g. group collapse,
    // filter), so a numeric index would point to an unrelated row after the
    // rebuild.
    let topRow = null
    let topOffset = 0
    ...
    for (let i = this.vDomTop; i <= this.vDomBottom; i++) {
      if (rows[i]) {
        ...
        if (topRow === null || Math.abs(diff) < Math.abs(topOffset)) {
          topOffset = diff
          topRow = rows[i]
        } else {
          break
    ...
    if (newRows.length) {
      let resolvedIndex = -1
      let resolvedOffset = topOffset

      if (topRow) {
        resolvedIndex = newRows.indexOf(topRow)

        if (resolvedIndex === -1) {
          // Anchor was removed (e.g. row inside collapsed group, or a Group
          // instance regenerated by `generateGroups`). Walk the previous
          // viewport upward through the OLD rows array to find the nearest
          // surviving row. Read its old offsetTop from the still-attached DOM
          // so the new anchor is placed where it visually sat before.
          const oldIdx = rows.indexOf(topRow)
          for (let i = oldIdx - 1; i >= 0; i--) {
            const candidate = newRows.indexOf(rows[i])
            if (candidate !== -1) {
              resolvedIndex = candidate
              const candEl = rows[i].getElement()
              resolvedOffset = candEl ? scrollTop - candEl.offsetTop : 0
              break
            }
          }
        }
      ...
      if (resolvedIndex === -1) {
        resolvedIndex = topRow ? 0 : newRows.length - 1
        resolvedOffset = 0
      }

      this._virtualRenderFill(resolvedIndex, true, resolvedOffset)

      // `_virtualRenderFill` reconstructs scrollTop from `vDomRowHeight`, an
      // average estimate that may differ significantly from real row heights
      // (especially with group headers). Correct any drift by measuring where
      // the anchor actually landed and shifting scrollTop so it sits at the
      // intended relative viewport position.
      const anchor = newRows[resolvedIndex]
      const anchorEl = anchor && anchor.getElement && anchor.getElement()
      if (anchorEl && anchorEl.parentNode) {
        const expectedRelative = -resolvedOffset
        const actualRelative = anchorEl.offsetTop - this.elementVertical.scrollTop
        const correction = actualRelative - expectedRelative
        if (correction !== 0) {
          const target = Math.max(
            0,
            Math.min(
              this.elementVertical.scrollTop + correction,
              this.elementVertical.scrollHeight - this.elementVertical.clientHeight
            )
          )
          this.elementVertical.scrollTop = target
          this.scrollTop = target
          this.vDomScrollPosTop = target
          this.vDomScrollPosBottom = target
        }
      }
    } else {
      this.clear()
  ...
  visibleRows(includingBuffer) {
    ...
    if (includingBuffer) {
      ...
      for (let i = this.vDomTop; i <= this.vDomBottom; i++) {
        if (rows[i]) {
          // Read offsetTop once per row to avoid repeated layout flushes.
          const rowOffsetTop = rows[i].getElement().offsetTop
          if (!topFound) {
            if (topEdge - rowOffsetTop >= 0) {
              topRow = i
            } else {
              topFound = true

              if (bottomEdge - rowOffsetTop >= 0) {
                bottomRow = i
              } else {
                break
              }
