...
import rowSelectionFormatter from './extensions/formatters/rowSelection'
...
export default class SelectRow extends Module {
  ...
  static moduleExtensions = {
    format: {
      formatters: {
        rowSelection: rowSelectionFormatter
      }
    }
  }
      ...
      if (e.ctrlKey || e.metaKey) {
        toggledRows.forEach((toggledRow) => {
          if (toggledRow !== this.lastClickedRow) {
            if (this.table.options.selectableRows !== true && !this.isRowSelected(toggledRow)) {
  ...
  deselectRows(rows, silent) {
    ...
    if (Array.isArray(rowMatch)) {
      if (rowMatch.length) {
        rowMatch.forEach((row) => {
          change = this._deselectRow(row, true)
    ...
    if (rowMatch) {
      this._deselectRow(rowMatch, silent)
    }
  }
