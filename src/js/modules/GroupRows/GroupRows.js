...
export default class GroupRows extends Module {
  ...
  getChildGroups(group) {
    ...
    group.groupList.forEach((child) => {
      if (child.groupList.length) {
        groupComponents.push(...this.getChildGroups(child))
      } else {
  ...
  pullGroupListData(groupList) {
    ...
    groupList.forEach((group) => {
      ...
      if (group.hasSubGroups) {
        ...
        groupHeader.rowCount = childData.filter((item) => item.headerContent === undefined).length
  ...
  updateGroupRows(force) {
    ...
    if (!this.blockRedraw) {
      this.groupList.forEach((group) => {
        output.push(...group.getHeadersAndRows())
      })
