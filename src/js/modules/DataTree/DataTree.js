...
export default class DataTree extends Module {
  ...
  columnMoving() {
    ...
    this.table.rowManager.rows.forEach((row) => {
      rows.push(...this.getTreeChildren(row, false, true))
    })
      ...
      if (this.branchEl) {
        ...
        if (this.table.rtl) {
          config.branchEl.style.marginRight =
            (config.branchEl.offsetWidth + (parseFloat(window.getComputedStyle(config.branchEl).marginRight) || 0)) *
              (config.index - 1) +
            config.index * this.indent +
            'px'
        } else {
          config.branchEl.style.marginLeft =
            (config.branchEl.offsetWidth + (parseFloat(window.getComputedStyle(config.branchEl).marginLeft) || 0)) *
              (config.index - 1) +
            config.index * this.indent +
            'px'
        }
  ...
  findChildIndex(subject, parent) {
    ...
    if (typeof subject === 'object') {
      ...
      match = parent.data[this.field].find((row) => row[this.table.options.index] === subject)
    }
