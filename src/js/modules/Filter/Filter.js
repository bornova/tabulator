...
export default class Filter extends Module {
  ...
  constructor(table) {
    ...
    this.headerFilterColumnSet = new Set() // O(1) membership check for headerFilterColumns
  ...
  generateHeaderFilterElement(column, initialValue, reinitialize) {
    ...
    if (field) {
      ...
      if (editor) {
        ...
        if (!reinitialize && !this.headerFilterColumnSet.has(column)) {
          this.headerFilterColumns.push(column)
          this.headerFilterColumnSet.add(column)
        }
  ...
  getFilters(all, ajax) {
    ...
    output.push(...this.filtersToArray(this.filterList, ajax))
