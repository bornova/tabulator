...
import defaultReaders from './defaults/readers'
import defaultWriters from './defaults/writers'
...
export default class Persistence extends Module {
  ...
  mergeDefinition(oldCols, newCols, mergeAllNew) {
    ...
    oldCols.forEach((column, i) => {
      ...
      if (!from) {
        // find insert position: after the last preceding oldCols column that appears in output
        let insertPos = output.length
        for (let j = i - 1; j >= 0; j--) {
          const prev = this._findColumn(output, oldCols[j])
          if (prev) {
            insertPos = output.indexOf(prev) + 1
            break
          }
        }
        output.splice(insertPos, 0, column)
      }
    })
