...
import defaultSenders from './defaults/senders'
import defaultReceivers from './defaults/receivers'
...
export default class MoveRows extends Module {
  ...
  constructor(table) {
    ...
    this.movingRows = [] // all rows being moved (includes this.moving; multi-select support)
  ...
  startMove(e, row) {
    ...
    // Collect all rows to move: if the dragged row is selected and selectRow module is
    // active, move every selected row together (same-table only).
    if (
      !this.connection &&
      this.table.modExists('selectRow') &&
      this.table.modules.selectRow.selectedRows.includes(row)
    ) {
      const selected = this.table.modules.selectRow.selectedRows
      this.movingRows = this.table.rowManager.getDisplayRows().filter((r) => r.type === 'row' && selected.includes(r))
    } else {
      this.movingRows = [row]
    }
    ...
    if (!this.connection) {
      ...
      // Hide secondary selected rows during drag so only the placeholder is visible.
      this.movingRows.forEach((r) => {
        if (r !== row) {
          r.getElement().style.display = 'none'
        }
      })
    } else {
    ...
    if (this.connection) {
      ...
      // Show a count badge on the hover element when moving multiple rows.
      if (this.movingRows.length > 1) {
        const badge = document.createElement('div')
        badge.classList.add('tabulator-moving-count')
        badge.textContent = this.movingRows.length

        let cursorXPosition = e.pageX

        badge.style.left = `${cursorXPosition - this.hoverElement.getBoundingClientRect().left}px`

        this.hoverElement.appendChild(badge)
      }
    }
    ...
    if (!e || e.button === 0 || this.touchMove) {
      ...
      if (!this.connection) {
        if (this.placeholderElement.parentNode) {
          ...
          // Remove secondary moving rows from the DOM so the insertion reference
          // (placeholder.nextSibling) is not obscured by hidden rows.
          this.movingRows.forEach((r) => {
            if (r !== this.moving) {
          ...
          const insertRef = this.placeholderElement.nextSibling

          // Re-insert all moving rows in display order at the placeholder position.
          this.movingRows.forEach((r) => {
            const el = r.getElement()
            el.style.display = ''
          ...
          // Placeholder is not in DOM; just restore visibility of hidden rows.
          this.movingRows.forEach((r) => {
            if (r !== this.moving) {
              r.getElement().style.display = ''
            }
          })
        }
      }
      ...
      if (this.toRow) {
        // Move all rows to the target position in display order.
        // Each row becomes the anchor for the next, preserving relative order.
        let anchor = this.toRow
        let after = this.toRowAfter

        this.movingRows.forEach((r) => {
          this.table.rowManager.moveRow(r, anchor, after)
          anchor = r
          after = true
        })
      } else {
        this.movingRows.forEach((r) => {
          this.dispatchExternal('rowMoveCancelled', r.getComponent())
        })
      }
